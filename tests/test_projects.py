...
def test_create_project(auth_user):
    response = auth_user.post(reverse('project-list'), {'name':'My Project'})
...
def test_update_project(auth_user, user_projects):
    response = auth_user.put(reverse('project-detail', kwargs={'pk': user_projects[0].id}), {'name':'New Name'})
    assert response.status_code == 200
    assert response.json()['name'] == 'New Name'

@pytest.mark.django_db
def test_update_project_failure_permission(auth_user, user_projects, auth_user2):
    response = auth_user2.put(reverse('project-detail', kwargs={'pk': user_projects[0].id}), {'name': 'New Name'})
...
def test_delete_project(auth_user, user_projects):
    response = auth_user.delete(reverse('project-detail', kwargs={'pk': user_projects[0].id}))
